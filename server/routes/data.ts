...
router.get('/export', (req: Request, res: Response) => {
  try {
    ...
    const data = {
      vehicles: rowsToCamelCase(db.prepare('SELECT * FROM vehicles WHERE user_id = ?').all(userId) as any[]),
      costs: rowsToCamelCase(db.prepare('SELECT * FROM costs WHERE user_id = ?').all(userId) as any[]),
      loans: rowsToCamelCase(db.prepare('SELECT * FROM loans WHERE user_id = ?').all(userId) as any[]),
      repairs: rowsToCamelCase(db.prepare('SELECT * FROM repairs WHERE user_id = ?').all(userId) as any[]),
      savingsGoals: rowsToCamelCase(db.prepare('SELECT * FROM savings_goals WHERE user_id = ?').all(userId) as any[]),
      savingsTransactions: rowsToCamelCase(db.prepare('SELECT * FROM savings_transactions WHERE user_id = ?').all(userId) as any[]),
      plannedPurchases: rowsToCamelCase(db.prepare('SELECT * FROM planned_purchases WHERE user_id = ?').all(userId) as any[]),
      persons: rowsToCamelCase(db.prepare('SELECT * FROM persons WHERE user_id = ?').all(userId) as any[]),
    };
    ...
    const importAll = db.transaction(() => {
      ...
      if (Array.isArray(data.vehicles)) {
        for (const v of data.vehicles) {
          db.prepare(`INSERT INTO vehicles (id, user_id, name, brand, model, variant, license_plate, hsn, tsn, first_registration, purchase_price, purchase_date, current_mileage, annual_mileage, fuel_type, avg_consumption, fuel_price, horse_power, image_url, status, mobile_de_link, notes, color, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      if (Array.isArray(data.costs)) {
        for (const c of data.costs) {
          db.prepare(`INSERT INTO costs (id, user_id, vehicle_id, name, category, amount, frequency, paid_by, start_date, end_date, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      if (Array.isArray(data.loans)) {
        for (const l of data.loans) {
          db.prepare(`INSERT INTO loans (id, user_id, vehicle_id, name, total_amount, monthly_payment, interest_rate, start_date, duration_months, additional_savings_per_month, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      if (Array.isArray(data.repairs)) {
        for (const r of data.repairs) {
          db.prepare(`INSERT INTO repairs (id, user_id, vehicle_id, date, description, category, notes, cost, mileage, workshop, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      if (Array.isArray(data.savingsGoals)) {
        for (const g of data.savingsGoals) {
          db.prepare(`INSERT INTO savings_goals (id, user_id, vehicle_id, name, target_amount, monthly_contribution, start_date, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      if (Array.isArray(data.savingsTransactions)) {
        for (const t of data.savingsTransactions) {
          db.prepare(`INSERT INTO savings_transactions (id, user_id, savings_goal_id, date, amount, type, description) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      if (Array.isArray(data.plannedPurchases)) {
        for (const p of data.plannedPurchases) {
          db.prepare(`INSERT INTO planned_purchases (id, user_id, brand, model, variant, price, mobile_de_link, image_url, year, mileage, fuel_type, horse_power, down_payment, financing_months, interest_rate, monthly_rate, estimated_insurance, estimated_tax, estimated_fuel_monthly, estimated_maintenance, notes, pros, cons, rating, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
