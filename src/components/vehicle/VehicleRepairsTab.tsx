...
import type { AppState, Repair, Vehicle } from '../../types';
...
export default function VehicleRepairsTab({ vehicleId, state, setState }: Props) {
  const { t } = useI18n();
  ...
  const handleSave = async () => {
    try {
      let recordId: string;
      if (editingId) {
        const updated = await api.updateRepair(editingId, { ...form, vehicleId });
        setState({ ...state, repairs: state.repairs.map((r) => (r.id === editingId ? updated : r)) });
        recordId = editingId;
      } else {
        const created = await api.createRepair({ ...form, vehicleId });
        setState({ ...state, repairs: [...state.repairs, created] });
        recordId = created.id;
      }
      ...
      for (const sel of selectedSupplies) {
        try {
          await api.requisitionSupply(sel.supplyId, {
            ...
            recordType: 'repair',
            recordId,
            description: form.description || 'Repair',
          });
        } catch {
      ...
      if (createReminder && reminderDate) {
        try {
          await api.createReminder({
            title: `Repair: ${form.description || 'Untitled'}`,
            description: `Reminder for repair record`,
            type: 'custom',
            entityType: 'repair',
            entityId: recordId,
      ...
      console.error('Failed to save repair', e);
    }
  };
  ...
  const handleMove = async (recordId: string, destType: 'services' | 'upgrades') => {
    try {
      await api.moveRecords([recordId], 'repairs', destType);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <p className="text-sm text-zinc-400">
          {t('vehicle_tab.repairs.count', { count: records.length })} &middot; {t('common.total')}: {formatCurrency(totalCost)}
        </p>
        <button
          onClick={openAdd}
        ...
        >
          <Plus size={16} />
          {t('vehicle_tab.repairs.add')}
        </button>
      </div>

      {records.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 text-center">
          <p className="text-zinc-500 text-sm">{t('vehicle_tab.repairs.no_repairs')}</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-zinc-800">
                  ...
                  <th className="px-4 py-3.5 text-left text-xs text-zinc-500 uppercase tracking-wider font-medium">{t('repairs.workshop')}</th>
              ...
              <tbody>
                {records.map((r) => (
                  <tr key={r.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-sm text-zinc-400">{r.category || '-'}</td>
                    ...
                    <td className="px-4 py-3.5 text-sm text-zinc-400">{r.workshop || '-'}</td>
                          ...
                          {moveMenuOpen === r.id && (
                            <>
                              ...
                              <div className="absolute right-0 mt-1 w-44 bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl z-20 py-1">
                                ...
                                >
                                  {t('vehicle_tab.repairs.move_to_services')}
                                </button>
                                <button
                                  onClick={() => handleMove(r.id, 'upgrades')}
                                  className="w-full text-left px-4 py-2 text-sm text-zinc-300 hover:bg-zinc-800 transition-colors"
                                >
                                  {t('vehicle_tab.repairs.move_to_upgrades')}
                                </button>
                              </div>
                            </>
                          )}
      ...
      <Modal
        ...
        title={editingId ? t('vehicle_tab.repairs.edit') : t('vehicle_tab.repairs.add')}
        footer={
          <>
            <button
      ...
      >
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>{t('vehicle_tab.repairs.cost_eur')}</label>
              <input
                type="number"
                step="0.01"
                className={inputClass}
          ...
          <div>
            <label className={labelClass}>{t('repairs.workshop')}</label>
            <input
              type="text"
              className={inputClass}
