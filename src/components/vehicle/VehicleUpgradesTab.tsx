    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <p className="text-sm text-zinc-400">
          {records.length} upgrade{records.length !== 1 ? 's' : ''} &middot; Total: {formatCurrency(totalCost)}
        </p>
        <button
          onClick={openAdd}
      ...
      {records.length === 0 ? (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 text-center">
          <p className="text-zinc-500 text-sm">No upgrades recorded yet.</p>
        </div>
      ) : (
      ...
      <Modal
        ...
        title={editingId ? 'Edit Upgrade' : 'Add Upgrade'}
        footer={
          <>
            <button
